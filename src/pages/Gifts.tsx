...
import type { TelegramAvailableGift, TelegramGiftCardBackground, TelegramOwnedGift } from '../services/user/client'

function formatSendDate(unix?: number) {
...
function giftTitle(g: TelegramOwnedGift): string {
  if (g.kind === 'regular') {
    if (g.text?.trim()) return g.text.trim()
    if (g.giftId) return `Gift ${String(g.giftId).slice(0, 20)}`
    return 'Regular gift'
  }
  if (g.kind === 'unique') {
    const n = g.name || g.baseName || 'Unique gift'
    return g.number != null ? `${n} #${g.number}` : n
  }
  return 'Gift'
}

function giftSubtitle(g: TelegramOwnedGift): string {
  if (g.kind === 'regular') {
    const from = g.senderUser
    if (from?.first_name || from?.username) {
      return `From ${from.first_name || ''} ${from.username ? `@${from.username}` : ''}`.trim()
    }
  ...
  if (g.kind === 'unique') {
    ...
    return 'Unique Telegram gift'
  }
  return ''
}

function giftCardSurfaceStyle(bg: TelegramGiftCardBackground | null | undefined): React.CSSProperties {
...
/** Minimum list price (Stars) so seller net ≥ 1 after rounded platform % fee and Telegram send cost (matches server). */
function minListingPriceStars(telegramStarCost: number, platformFeePercent: number): number {
  const T = Math.max(0, Math.floor(telegramStarCost))
  const F = Math.max(0, Number(platformFeePercent) || 0)
  for (let P = T + 1; P < T + 100000; P++) {
    const fee = Math.round((P * F) / 100)
    if (P - fee - T >= 1) return P
  }
  return T + 2
}

function GiftStickerThumb({
  initData,
  fileId,
  emoji,
}: {
  ...
  if (showEmoji || !objectUrl) {
    return (
      <span className="text-[2.75rem] leading-none select-none drop-shadow-sm" aria-hidden>
        {emoji}
      </span>
    )
  }
