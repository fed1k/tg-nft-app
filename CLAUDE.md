...
**GiftedForge** is a Telegram Mini App NFT marketplace with TON and EVM blockchain support. It is split into a React frontend and an Express/MongoDB backend.

### Frontend (`src/`)

Entry point is [src/main.jsx](src/main.jsx), which sets up React Router and wraps the app in:
- `TelegramProvider` — wraps `window.Telegram.WebApp`, provides user identity from `initData`, handles Stars payment callbacks, and detects ban/suspend status. Calls `POST /api/user/session` on startup to sync the user record and check block status.
- `LanguageProvider` — i18n context for `en`/`ru`. Translations live in [src/i18n/translations.ts](src/i18n/translations.ts). Call `const { t } = useLanguage()` to get the translate function. Language preference is persisted to `localStorage` under key `gf_lang`.
- `TonConnectUIProvider` — TON wallet connection
- `WagmiProvider` + `RainbowKitProvider` — EVM wallet connection (mainnet, polygon, BSC, base)
- `QueryClientProvider` — React Query for all server state

Routes split into `/app/*` (user-facing, guarded by `RequireAppAccess`) and `/admin/*` (guarded by `RequireAdmin`). The `/offers` and `/swap` paths are top-level outside `/app`.

API calls use plain `fetch` via [src/services/user/client.ts](src/services/user/client.ts) and [src/services/admin/client.ts](src/services/admin/client.ts). Both clients send `initData` in the `X-Telegram-Init-Data` header. The admin client **also** appends `initData` as a query param (`?initData=...`).

Blockchain payloads are built off-chain in [src/utils/tonNft.ts](src/utils/tonNft.ts) and [src/utils/tonCollection.ts](src/utils/tonCollection.ts) using `@ton/core`, then sent via the connected wallet.
